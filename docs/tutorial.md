...
![Stacks homepage](/docs/images/stacks-homepage.png)
...
To create and debug the smart contracts locally, we are going to need these softwares installed on your machine:
...
To install the Clarinet on Windows using Winget, use the following command:
...
- Open the *Extensions* sidebar, and search “*Clarity for Visual Studio Code*”
...
If you have successfully installed the Clarinet, you can start to develop the Clarinet smart contract by creating a project using the following command:
...
The command above will create a Clarity project directory and populate it with the boilerplates that will help you to speed up the development process.
...
Clarity features a *LISP-like* syntax, meaning that you will see a lot of parentheses in Clarity, and inside these parentheses, there will be a lot of symbols, phrases, values, and other parentheses. Do not be intimidated by this if you find yourself un-familiar with the syntax. Everyone has to start somewhere.

### Interacting With The Smart Contract

In this step, we are going to use Clarinet console that you have installed in the previous section in this tutorial. To begin with, we need to verify that the contract is correct with this command:
...
What we have just done was, we are calling the built-in `contract-call` function, and it opened the `my-contract` file, and call the `say-hi` function.
...
But, what about arguments? At some point, we are going to need to put an argument for more complex function.

Let’s get back to `my-contract.clar` and write these lines of code below the `say-hi` function, and name it `my-name` function.
...
As you can see in the `my-name` function, the first line of this function contains a lot of parentheses, similar to LIPS language.

The final smart contract file would be like this:
...
Since the `my-name` function takes an argument, we can put the argument after the function name, hence the `"John"` in the end of command before parentheses.
...
That is, we are now understand how to write a Clarity smart contract and how to interact with. The next step of this tutorial is to deploy the smart contract to testnet, and interact with it. But before we jump into that part, it is better for us to understand the basic of Clarity.
...
To install Hiro Wallet on your web browser, you can start by opening the official download page at [https://wallet.hiro.so/wallet/install-web](https://wallet.hiro.so/wallet/install-web). At the point when this tutorial is written, Hiro Wallet only supports Chrome/Brave and Firefox, so if you are using another browser, you have to switch your browser. 

Just like the Clarinet, after you installed the wallet, you will be asked if you are willing to participate in the Hiro Wallet development by sending diagnostic and usage data. Click “*Allow”* if you want to participate, or click “*Deny”*  if you do not want to participate.
...
- The wallet will generate a new secret phrase. Treat it as a password and save it in somewhere safe. Click “*I’ve backed it up”* button if you have stored it.
...
- You will be asked to fund your wallet. You can click “*Skip*” button in the top-right corner.
- You are set! You wallet is ready to use.

### Request STX Balance

Every single transaction in blockchain requires you to pay with real money (that already converted to cryptocurrency). But in development phase, we do not want to spend our money just to test the application. That is why the testnet exists; to help you develop, test and interact with the application (in this case is smart contract) without spending any amount of real money. Think that testnet is a sandbox environment and you can do anything inside it without worrying the real-world consequence. 
...
All you need to do is to wait patiently for the fund to be transferred to your wallet. You can also monitor the transaction from your wallet. Click your wallet extension, and click on the “*Activity”* tab on your wallet.
...
The first step you need to do is to open the link to open the *Write & Deploy* section of the sandbox application. Open this link [https://explorer.stacks.co/sandbox/deploy?chain=testnet](https://explorer.stacks.co/sandbox/deploy?chain=testnet). By default, you will see this page:
...
You can also click the transaction item in your wallet to open it in the Stacks’ blockchain explorer to see the latest status of your transaction. You can also grab the smart contract address from this explorer page, which we will be using later.
...
Again, please do not be discouraged with the pre-filled fields in this page. In this page, the only thing you need to first is to grab your smart contract address along with the smart contract name.

The smart contract address can be retrieved from you wallet’s activity, or the easier part is opening the transaction history by clicking the hamburger menu (three horizontal lines) in top-right corner of the sandbox application.
...
Then, you click on your `my-contract` listed in the *Activity* list in your wallet, and it will open a new page. The `my-contract` is the smart contract name you have wrote in the previous part of deploying smart contract, and it could be different.
...
After the smart contract address and smart contract name pasted to the correct field, click the “*Get contract*” button, and it will load your smart contract. The page will be changed to something like this:
...
The difference between the `my-name` and `say-hi` function is, the `my-name` function does not call a pop-up wallet window. This is because that we are not triggering any on-chain transaction in the blockchain in the `my-name` function.
...
We have reached the beginning of final chapter, and you will be guided to build a decentralized application using ReactJS and Clarity. The application we are going to build is an on-chain calculator which can perform basic math operations (addition, subtraction, division, and multiplication) and deploy it to the Stacks testnet. 
...
It is better to start the development from the smart contract, as smart contract is the core of our decentralized application. The smart contract will contain several functions to perform basic math operation.
...
- The connected wallet address (*currently hidden*)
- The field to put the numbers and choose the operator
- Buttons to connect/disconnect wallet and calculate the result
- Link to the transaction in the blockchain explorer (*currently hidden*)
...
Before we start to write the code, we need to setting up the ReactJS and TailwindCSS. The TailwindCSS is optional if you do not want to use it, but you have to write your own CSS code if you choose not to use TailwindCSS.
...
If there is no error, you should see this screen:

![stacalc-1.png](/docs/images/stacalc-1%201.png)

There is a *Connect Wallet* button next to a disabled *Calculate* button. The first thing you need to do is clicking the *Connect Wallet* button in order to connect your Hiro Wallet to be able to create a transaction.
...
Click *Confirm* button on the pop-up window to execute the transaction. A new *Click to see result* link below the buttons will appear, and you can check the transaction in the blockchain explorer by clicking the link.
...
The result of `12 + 33` is `45` and it is recorded in the blockchain. At this point, you have successfully create your first decentralized application on Stacks blockchain.

Congratulation!

# Conclusion

You have reached the end of this tutorial, and if you follow this guide carefully, you will:
